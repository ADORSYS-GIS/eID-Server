...
async fn test_send_signed_saml_request() {
    ...
    encoder.write_all(signed_xml.as_bytes()).expect("Deflate failed");
    let compressed = encoder.finish().into_result().expect("Deflate finish failed");
    ...
    let relay_state = format!(
        "t_C5eLjqYDiBmqsL_jzsdaoqf5z8DBEIieoaRVH6aAk.y9HSXm5nrPs.xVIpjU21QTWs6N-HTtTldw.eyJydSI6Imh0dHBzOi8vbG9jYWxob3N0Ojg0NDMvYWRtaW4vbWFzdGVyL2NvbnNvbGUvIiwicnQiOiJjb2RlIiwicm0iOiJxdWVyeSIsInN0IjoiMWY4NDdhMjctZTMyNC00OTMwLTg4NTMtYTQyMzdjYWNkYmE3In0.{}",
        timestamp
    );
    ...
    let private_key = fs::read_to_string("./tests/keys/saml_request_private_key.pem").expect("Failed to read private key");
    println!("Read private key successfully");
    
    let pkey = PKey::private_key_from_pem(private_key.as_bytes()).expect("Failed to parse private key");
    ...
    signer.update(string_to_sign.as_bytes()).expect("Failed to update signer");
