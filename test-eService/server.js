...
app.prepare().then(() => {
  if (isHttps) {
    ...
    const keyPath =
      process.env.HTTPS_KEY_PATH || path.join(__dirname, "certs", "client.key");
    const caPath = process.env.HTTPS_CA_PATH; // Optional

    try {
      if (!fs.existsSync(certPath) || !fs.existsSync(keyPath)) {
        throw new Error(
          "Certificate or key file not found at the specified paths.",
        );
      }
      ...
      console.log(
        "🔒 Enforcing modern TLS protocols and strong cipher suites.",
      );
      ...
      createHttpsServer(httpsOptions, (req, res) => {
        ...
        if (process.env.NODE_TLS_REJECT_UNAUTHORIZED === "0") {
          console.log(
            "> 🔒 Self-signed certificate acceptance is enabled for outgoing requests.",
          );
        }
      });
    } catch (error) {
