...
export default async function handler(
  ...
  try {
    ...
    if (!session.sessionId) {
      console.log(
        `Configuration session found, calling useID for token: ${token}`,
      );
      ...
      const tlsOptions: any = {
        ...
        mode: (process.env.EID_SERVER_TLS_MODE === "mtls"
          ? "mtls"
          : "normal") as "mtls" | "normal",
      ...
      if (!useIDResponse.Result.ResultMajor.includes("#ok")) {
        console.log(
          `useID failed for token ${token}:`,
          useIDResponse.Result.ResultMajor,
        );
        return res
          .status(400)
          .send(
            `eID-Server returned error: ${useIDResponse.Result.ResultMajor}`,
          );
      }
    ...
    const tcTokenWithoutDeclaration = xmlContent.replace(
      /<\?xml[^?]*\?>\s*/,
      "",
    );
