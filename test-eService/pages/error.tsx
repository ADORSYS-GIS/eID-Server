...
export default function ErrorPage() {
  ...
  const { token, ResultMajor, ResultMinor, ResultMessage, ResultMessageTLS } =
    router.query;
  ...
  useEffect(() => {
    ...
    if (ResultMajor && typeof ResultMajor === "string") {
      setErrorDetails({
        ...
        resultMessage:
          typeof ResultMessage === "string" ? ResultMessage : undefined,
        tlsAlertDescription:
          typeof ResultMessageTLS === "string" ? ResultMessageTLS : undefined,
      });
    }
  }, [ResultMajor, ResultMinor, ResultMessage, ResultMessageTLS]);

  const errorInfo = errorDetails
    ? getErrorInfo(errorDetails.resultMinor)
    : null;
          ...
          <div className="mb-8 space-y-4">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="space-y-1 text-sm">
                <div>
                  <span className="font-medium">Result Major:</span>{" "}
                  <span className="font-mono">{errorDetails.resultMajor}</span>
                </div>
