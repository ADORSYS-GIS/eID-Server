...
impl CardCommunicator {
    ...
    pub async fn send_did_authenticate(
        ...
        let response_body = body::to_bytes(response.into_body()).await.map_err(|e| {
            AuthError::CardCommunicationError {
                reason: format!("Failed to read response body: {}", e),
            }
        })?;

        let response_text = String::from_utf8(response_body.to_vec()).map_err(|e| {
            AuthError::CardCommunicationError {
