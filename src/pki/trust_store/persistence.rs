...
pub trait TrustStoreRepository {
    ...
    async fn load_certificates(
        &self,
    ) -> Result<HashMap<String, CSCAPublicKeyInfo>, TrustStoreError>;
...
impl TrustStoreRepository for FileTrustStoreRepository {
    async fn load_certificates(
        ...
        info!(
            "Attempting to load certificates from file: {:?}",
            self.file_path
        );
        if !self.file_path.exists() {
            info!(
                "No trust store file found at {:?}. Initializing with an empty store.",
                self.file_path
            );
        ...
        let combined_data = hex::decode(&content).map_err(|e| {
            TrustStoreError::DecryptionError(format!("Failed to decode hex: {}", e))
        })?;

        // Ensure combined_data has at least 12 bytes for the nonce
        if combined_data.len() < 12 {
            return Err(TrustStoreError::DecryptionError(
        ...
        let plaintext = String::from_utf8(plaintext_bytes).map_err(|e| {
            TrustStoreError::DecryptionError(format!("Failed to convert to string: {}", e))
        })?;
        ...
        info!(
            "Successfully loaded {} certificates from file: {:?}",
            certificates.len(),
            self.file_path
        );
        Ok(certificates)
    ...
    async fn save_certificates(
        ...
        info!(
            "Attempting to save {} certificates to file: {:?} (encrypted)",
            certificates.len(),
            self.file_path
        );
        ...
        info!(
            "Successfully saved certificates to file: {:?} (encrypted)",
            self.file_path
        );
        Ok(())
    }
